...
int *classification_global; 
//thread for finding ben_dist


void* dist_finder2(void* threadid) {
	int thrid = (int)threadid;  
	int single_chunk = DATAPOINTS/THREADS;
	int x = single_chunk*thrid;
	int y = (single_chunk*(thrid+1))-1;
	//printf("launched thread %d, x:%d, y:%d \n", thrid, x, y);
	float mal_dist, ben_dist;
	for (int i=x; i < y; i++) {
		//printf("t:%d, i:%d \n",thrid,i);
		...
		if (mal_dist < ben_dist) {
                	classification_global[i] = MALIGN;
                	total_mal_points++;
               		total_mal_distance += mal_dist;
        	} else {
                	classification_global[i] = BENIGN;
                	total_ben_points++;
 ...
 */
   for (int t=0; t< total_threads; t++) {
	...
	pthread_create(&tid[t], NULL, &dist_finder2, (void *)t);
   }
   for (int t=0; t< total_threads; t++) {
	 //printf("pthread_join %d\n",t);
	pthread_join(tid[t], NULL);   
   }
   //printf("finish classify_points\n");
 //  dist_finder2;
  // printf("classification 2 %d \n", classification[0]); 
}
